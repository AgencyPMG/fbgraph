...
Graph.prototype.get = function () {
  ...
  return request.get(this.options, (err, res, body) => {
    ...
    // if body is already an object, it means we parsed it or it was an image
    // if it's still a string, self.end will handle it (e.g. query strings)
    // in all successful cases, we want to attach headers
    var originalCallback = self.callback;
    self.callback = function(err, data) {
      if (!err && data && typeof data === 'object' && !data.headers) {
