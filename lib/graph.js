...
Graph.prototype.get = function () {
    ...
    return request.get(this.options, (err, res, body) => {
        ...
        if (res.headers['content-type'] && ~res.headers['content-type'].indexOf('image')) {
            return self.end({ image: true, headers: res.headers });
        }

        body = parseAndValidateJSON(self, body, callbackCalled);
        if (body === null) return;
...
/**
 * Wraps the graph callback to attach response headers
 * @param {object} self - the graph object
 * @param {object} res - the request response object
 */
function wrapCallbackWithHeaders(self, res) {
    var originalCallback = self.callback;
    self.callback = function(err, data) {
        ...
        originalCallback.call(self, err, data);
    };
}
 ...
 * @param {boolean} callbackCalled - whether the callback has been called
 * @returns {object|null} the parsed body or null if an error occurred
 */
function parseAndValidateJSON(self, body, callbackCalled) {
...
exports.__test__ = {
    ...
    parseAndValidateJSON: parseAndValidateJSON
};
